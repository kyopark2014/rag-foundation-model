"""RAG upload orchestration: S3 ingest + OpenSearch Knowledge Base sync.
...
OpenSearch Serverless supports STRING_LIST + listContains filtering
(unlike Neptune GraphRAG, which only allows STRING/NUMBER/BOOLEAN).
"""
...
def build_kb_metadata_document(
    *,
    ...
    ``owner`` uses STRING_LIST so Retrieve can use ``listContains`` (OpenSearch).
    All attributes set ``includeForEmbedding`` to false (filter-only).
    ``created_time`` is a Unix epoch in seconds (NUMBER) for range filters.
    """
...
def ingest_rag_upload(
    ...
    *,
    ...
    Foundation Model parser (BEDROCK_FOUNDATION_MODEL) processes documents during
    sync; metadata sidecars are indexed independently of the parser choice.
    """
    try:
