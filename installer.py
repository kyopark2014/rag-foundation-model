...
# Configuration
project_name = "rag-fm-parser" # at least 3 characters
region = "us-west-2"
...
git_name = "rag-foundation-model"  # Foundation Model Parser
cloudfront_comment = "CloudFront-for-rag-project"

sts_client = boto3.client("sts", region_name=region)
account_id = sts_client.get_caller_identity()["Account"]

# Parsing model for Knowledge Base data source (same as agent-skills)
    ...
    try:
        ...
        for collection in existing_collections.get("collectionSummaries", []):
            if collection["name"] == collection_name:
                logger.warning(
                    f"Reusing existing OpenSearch collection: {collection['name']} "
                    f"(status: {collection['status']})"
                )
                collection_arn = collection["arn"]

                # Get collection details and wait until ACTIVE with endpoint
                ...
                if collection_detail.get("status") != "ACTIVE" or not collection_endpoint:
                    ...
                    while wait_count < 60:
                        ...
                        if status == "ACTIVE" and collection_endpoint:
                            break
                    else:
                        raise Exception(
                            f"Timeout waiting for OpenSearch collection. Status: {status}"
                        )

                # If endpoint is still not available, wait for collection to be ready
...
def _foundation_model_vector_ingestion_configuration() -> Dict:
    """Vector ingestion config with Foundation Model parser (agent-skills compatible)."""
    return {
...
def create_knowledge_base_with_opensearch(opensearch_info: Dict[str, str], knowledge_base_role_arn: str, s3_bucket_name: str) -> str:
        ...
        for kb in kb_list.get("knowledgeBaseSummaries", []):
            if kb["name"] == project_name:
                ...
                if kb_collection_arn != opensearch_info["arn"]:
                    logger.warning("Knowledge Base is using wrong OpenSearch collection:")
    ...
    response = bedrock_agent_client.create_knowledge_base(
        name=project_name,
        description="Knowledge base with Foundation Model parser (OpenSearch Serverless)",
